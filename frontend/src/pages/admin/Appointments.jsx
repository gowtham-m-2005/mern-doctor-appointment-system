    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface-container-low rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="text-center py-12">
            <Calendar className="w-12 h-12 mx-auto mb-4" style={{color: 'color-mix(in srgb, var(--on-surface-variant) 30%, transparent)'}} />
            <p className="text-on-surface-variant">No appointments found</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-surface-container-high border-b style={{borderColor: 'color-mix(in srgb, var(--outline-variant) 10%, transparent)'}}">
              ...
              <tbody className="divide-y" style={{borderColor: 'color-mix(in srgb, var(--outline-variant) 10%, transparent)'}} key={`table-${statusFilter}-${dateFilter}`}>
                {dateFilteredAppointments.map((appt, index) => (
                  <tr key={appt._id} data-appointment-id={appt._id} className={`hover:bg-surface-container-highest/50 transition-colors animate-fade-in ${highlightAppointmentId === appt._id ? 'bg-primary/10' : ''}`} style={{ animationDelay: `${index * 30}ms` }}>
                    <td className="px-6 py-4">
                      <p className="font-medium text-sm text-on-surface">{appt.user?.name || '—'}</p>
                      <p className="text-xs style={{color: 'color-mix(in srgb, var(--on-surface-variant) 60%, transparent)'}}">{appt.user?.email}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-medium text-sm text-on-surface">{appt.doctor?.user?.name || '—'}</p>
                    </td>
                    <td className="px-6 py-4">
                      ...
                      <div className="flex items-center gap-1 text-xs mt-0.5" style={{color: 'color-mix(in srgb, var(--on-surface-variant) 60%, transparent)'}}>
